...
function formatSupportedDenomination(lamports: bigint): string {
  const sol = Number(lamports) / 1_000_000_000;
  return lamports % 100_000_000n === 0n ? sol.toFixed(1) : sol.toFixed(2);
}

export const SUPPORTED_DENOMINATION_DISPLAY = `${SUPPORTED_DENOMINATION_LAMPORTS.map(formatSupportedDenomination).join(" / ")} SOL`;
