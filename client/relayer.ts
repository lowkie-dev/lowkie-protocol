...
interface SubNoteWithdrawInfo {
  /** 32-byte note secret (encrypted for MPC, never sent as plaintext). */
  noteSecret: Uint8Array;
  /** 32-byte withdraw key (for on-chain recipient verification). */
  withdrawKey: Uint8Array;
  /** SHA256 note hash (commitment). */
  hash: Buffer;
  /** Pool denomination this note belongs to. */
  denominationLamports: bigint;
  /**
   * Expected lamport amount — used ONLY for client-side balance verification.
   * This value is NOT sent to the on-chain program. The program reads the
   * amount from NoteAccount.lamports_for_transfer instead.
   */
...
async function compactRegistryForDenomination(
  ...
  const sig = await params.program.methods
...
export async function lowkieWithdraw(p: WithdrawParams) {
  const relayerPath = p.relayerKeypairPath ?? process.env.RELAYER_KEYPAIR_PATH;
  if (!relayerPath) {
    throw new Error(
  ...
  const balBefore = await p.provider.connection.getBalance(p.recipient);
  ...
  for (let i = 0; i < p.subNotes.length; i++) {
    ...
    const maxAttempts = AUTO_COMPACT_REGISTRY ? MAX_AUTO_COMPACT_RETRIES : 1;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      console.log(`      ✅ Tx: ${sig}`);
      await awaitComputationFinalization(p.provider, offset, PROG, "confirmed");
      console.log("      ✅ MPC confirmed");

      const noteAfterWithdraw = await (
        p.program.account as any
      ).noteAccount.fetch(notePDA);
    ...
    const compactSig = await p.program.methods
  ...
  const balAfter = await p.provider.connection.getBalance(p.recipient);
  console.log(
...
if (require.main === module) {
  ...
  const relayerPath = process.env.RELAYER_KEYPAIR_PATH ?? DEFAULT_WALLET_PATH;
  const relayerKp = readKeypair(relayerPath);
  ...
  const subNotes: SubNoteWithdrawInfo[] = n.subNotes
